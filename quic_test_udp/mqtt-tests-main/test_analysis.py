...
if __name__ == '__main__':
    transports = os.listdir('./results_qos_0')
    pcap_results = []
    pcap_cols = None
    perf_results = []
    perf_cols = None
    for transport in transports:
        files = os.listdir('./results_qos_0/{}'.format(transport))
        files.sort()
        print('Results for transport', transport)
        for file in files:
            if 'pcap' in file:
                pcap_file = './results_qos_0/{}/{}'.format(transport, file)
                ...
                perf_file = './results_qos_0/{}/{}'.format(transport, file)
