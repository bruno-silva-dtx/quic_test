...
def publish(client, n_messages=5, msg_size=1, msg_interval=1, qos=0):
    msg_count = 1
    while True:
        msg = msg_size * "0"
...
def run(broker, transport, n_messages=5, msg_size=1, msg_interval=1, qos=0):
    client = connect_mqtt(broker, transport)
    client.loop_start()
    publish(client, n_messages, msg_size, msg_interval, qos)
    client.loop_stop()

if __name__ == '__main__':
    # create argument parser
    parser = argparse.ArgumentParser(description='MQTT publisher - Set the broker address, transport protocol, and port')
    ...
    parser.add_argument('-q', '--qos', help='MQTT QoS level (0, 1, 2)', type=int, choices=[0, 1, 2], default=0)
    ...
    qos = args.qos if args.qos else 0
    ...
    if args.broker:
        ...
        if args.transport in ['tcp', 'tls', 'ws', 'wss']:
            # run publisher
            run(args.broker, args.transport, int(n_messages), int(msg_size), float(msg_interval), qos)
        else:
            print(f'The transport {args.transport} is not supported')
    else:
