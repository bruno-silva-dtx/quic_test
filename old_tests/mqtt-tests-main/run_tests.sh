...
helpFunction()
{
   ...
   echo -e "$0 -r 10 -l 1% -p 25% -d 20ms -j 5ms -n 10 -s 100 -i 1 -q 0"
...
echo "Parameters: runs=$runs, loss=$loss, burstloss=$burstloss, delay=$delay, delaystddev=$delaystddev, number_of_packets=$number_of_packets, size_of_packets=$size_of_packets, msg_interval=$msg_interval, qos=$qos"
...
do
    ...
    sleep 5
    docker run --network=mqtt-tests-main_emqx-bridge -it mqtt-pub:0.1 -b broker.emqx.io -t tcp -n $number_of_packets -s $size_of_packets -i $msg_interval -q $qos
...
do
    ...
    sleep 5
    docker run --network=mqtt-tests-main_emqx-bridge -it mqtt-pub:0.1 -b broker.emqx.io -t tls -n $number_of_packets -s $size_of_packets -i $msg_interval  -q $qos
    pid=$(ps -e | pgrep tcpdump)  
    sleep 5
    sudo kill -2 $pid
    x=$(( $x + 1 ))
...
do
    ...
    sleep 5
    docker run --network=mqtt-tests-main_emqx-bridge -it mqtt-pub:0.1 -b broker.emqx.io -t ws -n $number_of_packets -s $size_of_packets -i $msg_interval  -q $qos
    pid=$(ps -e | pgrep tcpdump)  
    sleep 5
    sudo kill -2 $pid
    x=$(( $x + 1 ))
...
do
    ...
    sleep 5
    docker run --network=mqtt-tests-main_emqx-bridge -it mqtt-pub:0.1 -b broker.emqx.io -t wss -n $number_of_packets -s $size_of_packets -i $msg_interval  -q $qos
    pid=$(ps -e | pgrep tcpdump)  
    sleep 5
    sudo kill -2 $pid
    x=$(( $x + 1 ))
