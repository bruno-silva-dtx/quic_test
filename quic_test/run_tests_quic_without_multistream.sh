...
mkdir -p ./results_without_multistream/quic
mkdir -p ./results_without_multistream/quic/captures



sudo docker run -d --name quic_test_without_multistream --network=quic_test_without_multistream_emqx-bridge quic_test_without_multistream tail -f /dev/null

docker exec quic_test_without_multistream bash -c "
            ...
            "
docker cp send_msg_quic.sh quic_test_without_multistream:/root/NanoSDK/extern/msquic
docker cp quic_api.c quic_test_without_multistream:/root/NanoSDK/src/supplemental/quic
...
for (( x=1; x<=$runs; x++ )); do
      ...
      echo "Executando o tcmdump no quic_test_without_multistream"
      docker exec quic_test_without_multistream bash -c "
         apt install sudo -y && \
         apt install sudo -y && \
   ...
   # ./scripts/log_wrapper.sh  ./send_msg_quic.sh 0 topic 100 10 10
      ...
      docker cp quic_test_without_multistream:/root/NanoSDK/extern/msquic/log_tracer_${loss}_${delay}_${number_of_packets}_${msg_interval}_${qos}_${x}.log ./results_without_multistream/quic/log_tracer-$x-loss-$loss-delay-$delay-n-$number_of_packets-s-$size_of_packets-i-$msg_interval-q-$qos.log
      docker cp quic_test_without_multistream:/root/NanoSDK/extern/msquic/log_msquic_${loss}_${delay}_${number_of_packets}_${msg_interval}_${qos}_${x}.log ./results_without_multistream/quic/log_msquic-$x-loss-$loss-delay-$delay-n-$number_of_packets-s-$size_of_packets-i-$msg_interval-q-$qos.log
      docker cp quic_test_without_multistream:/tmp/SslKeyLogFile_cb  ./results_without_multistream/quic/SslKeyLogFile-$x-loss-$loss-delay-$delay-n-$number_of_packets-s-$size_of_packets-i-$msg_interval-q-$qos.txt
      docker cp quic_test_without_multistream:/tmp/run-$x-loss-$loss-delay-$delay-n-$number_of_packets-s-$size_of_packets-i-$msg_interval-q-$qos-client.pcap ./results_without_multistream/quic/captures/run-$x-loss-$loss-delay-$delay-n-$number_of_packets-s-$size_of_packets-i-$msg_interval-q-$qos-client.pcap
      docker cp emqx:/tmp/run-$x-loss-$loss-delay-$delay-n-$number_of_packets-s-$size_of_packets-i-$msg_interval-q-$qos-emqx.pcap ./results_without_multistream/quic/captures/run-$x-loss-$loss-delay-$delay-n-$number_of_packets-s-$size_of_packets-i-$msg_interval-q-$qos-emqx.pcap
      ...
      docker exec quic_test_without_multistream bash -c "kill -9 \$(cat /tmp/tcpdump-quic-test-$x-$loss.pid)"
...
# Clean up docker
docker stop quic_test_without_multistream quic_test_without_multistream_receiver
docker rm quic_test_without_multistream quic_test_without_multistream_receiver
#docker compose down
