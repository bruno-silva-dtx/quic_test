...
// Obtém o valor da variável de ambiente SSLKEYLOGFILE
const char* SslKeyLogFile = "//tmp/SSLKEYLOGFILE";
...
void WriteSslKeyLogFileUserMode(
    ...
    char ClientRandomBuffer[(2 * sizeof(TlsSecrets.ClientRandom)) + 1] = {0};
	...
	_Inout_ QUIC_CONNECTION_EVENT *Event)
{
	...
	case QUIC_CONNECTION_EVENT_CONNECTED:
		...
		log_info("[conn][%p] is Connected. Resumed Session %d", qconn,
    		...
    		if (SslKeyLogFile != NULL) {
        		WriteSslKeyLogFileUserMode("/tmp/SslKeyLogFile_cb", ClientSecrets);
   		 	}
	...
	case QUIC_CONNECTION_EVENT_SHUTDOWN_INITIATED_BY_TRANSPORT:
		...
		// the connection.
		if (SslKeyLogFile != NULL) {
        		WriteSslKeyLogFileUserMode("/tmp/SslKeyLogFile_shutdown", ClientSecrets);
   		 	}
...
quic_connect_ipv4(const char *url, nni_sock *sock, uint32_t *index, void **qsockp)
{
	...
	if (SslKeyLogFile != NULL) {
    MsQuic->SetParam(conn, QUIC_PARAM_CONN_TLS_SECRETS, sizeof(ClientSecrets), &ClientSecrets);
    // Check for errors...
	};
	if (SslKeyLogFile != NULL) {
        		WriteSslKeyLogFileUserMode("/tmp/SslKeyLogFile_ipv4", ClientSecrets);
   	}
...
quic_pipe_send(void *qpipe, nni_aio *aio)
{
	...
	if (SslKeyLogFile != NULL) {
        		WriteSslKeyLogFileUserMode("/tmp/SslKeyLogFile_send", ClientSecrets);
   	}
	quic_strm_t *qstrm = qpipe;
	int          rv;
...
{
	if (SslKeyLogFile != NULL) {
        		WriteSslKeyLogFileUserMode("/tmp/SslKeyLogFile_close", ClientSecrets);
   		 	}
	if (MsQuic != NULL) {
