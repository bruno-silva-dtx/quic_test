...
helpFunction()
{
   ...
   echo -e "$0 -r 10 -l 0% -p 0% -d 20ms -j 5ms -n 1 -s 1500 -i 1 -q 0"
...
sudo docker run -d --name quic_test_tls --network=quic_mqtt_tls_emqx-bridge quic_test_tls tail -f /dev/null



docker cp send_msg_quic.sh quic_test_tls:/root/NanoSDK/extern/msquic
docker cp certs quic_test_tls:/root/quic_mqtt

docker exec quic_test_tls bash -c " 
...
for (( x=1; x<=$runs; x++ )); do
      ...
      echo "Executando o tcmdump no quic_test_tls"
      docker exec quic_test_tls bash -c "
         apt install sudo -y && \
         apt install sudo -y && \
      ...
      docker exec quic_test_tls bash -c "
         ...
         lttng destroy msqui && \ 
         ./scripts/log_wrapper.sh ./send_msg_quic.sh ${qos} topic ${size_of_packets} ${number_of_packets} ${msg_interval} > log_tracer_${loss}_${delay}_${number_of_packets}_${msg_interval}_${qos}_${x}.log 
      "  
      ...
      docker cp quic_test_tls:/root/NanoSDK/extern/msquic/log_tracer_${loss}_${delay}_${number_of_packets}_${msg_interval}_${qos}_${x}.log ./results/quic/log_tracer-$x-loss-$loss-delay-$delay-n-$number_of_packets-s-$size_of_packets-i-$msg_interval-q-$qos.log
      docker cp quic_test_tls:/root/NanoSDK/extern/msquic/log_msquic_${loss}_${delay}_${number_of_packets}_${msg_interval}_${qos}_${x}.log ./results/quic/log_msquic-$x-loss-$loss-delay-$delay-n-$number_of_packets-s-$size_of_packets-i-$msg_interval-q-$qos.log
      docker cp quic_test_tls:/tmp/SslKeyLogFile_cb  ./results/quic/SslKeyLogFile-$x-loss-$loss-delay-$delay-n-$number_of_packets-s-$size_of_packets-i-$msg_interval-q-$qos.txt
      docker cp quic_test_tls:/tmp/run-$x-loss-$loss-delay-$delay-n-$number_of_packets-s-$size_of_packets-i-$msg_interval-q-$qos-client.pcap ./results/quic/captures/run-$x-loss-$loss-delay-$delay-n-$number_of_packets-s-$size_of_packets-i-$msg_interval-q-$qos-client.pcap
      ...
      docker exec quic_test_tls bash -c "kill -9 \$(cat /tmp/tcpdump-quic-test-$x-$loss.pid)"
...
# Clean up docker
docker stop quic_test_tls quic_test_tls_receiver
docker rm quic_test_tls quic_test_tls_receiver
#docker compose down
